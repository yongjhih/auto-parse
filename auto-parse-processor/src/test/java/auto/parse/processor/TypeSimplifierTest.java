...
/**
 * Tests for {@link auto.parse.processor.TypeSimplifier}.
 *
...
public class TypeSimplifierTest extends TestCase {
  ...
  private void doTestTypeSimplifier(File tmpDir) throws Exception {
    ...
    List<String> options =
        ImmutableList.of("-sourcepath", tmpDir.getPath(), "-d", tmpDir.getPath(), "-processor",
    ...
    JavaFileObject sourceFile =
        fileManager.getJavaFileForInput(StandardLocation.SOURCE_PATH, TEST_JAVA_CLASS_NAME,
            Kind.SOURCE);
    ...
    JavaCompiler.CompilationTask javacTask =
        javac.getTask(compilerOut, fileManager, diagnosticCollector, options,
  ...
  public static class TestProcessor extends AbstractProcessor {
    ...
    public void testImportsForImplicitlyImportedTypes() {
      Set<TypeMirror> types = ImmutableSet.of(typeMirrorOf("java.lang.String"),
          typeMirrorOf("javax.management.MBeanServer"),  // Same package, so no import.
          typeUtil.getPrimitiveType(TypeKind.INT), typeUtil.getPrimitiveType(TypeKind.BOOLEAN));
    ...
    public void testImportsForPlainTypes() {
      Set<TypeMirror> types =
          ImmutableSet.of(typeUtil.getPrimitiveType(TypeKind.INT), typeMirrorOf("java.lang.String"),
              typeMirrorOf("java.util.Map"), typeMirrorOf("java.util.Map.Entry"),
              typeMirrorOf("java.util.regex.Pattern"),
              typeMirrorOf("javax.management.MBeanServer"));
      List<String> expectedImports =
          ImmutableList.of("java.util.Map", "java.util.Map.Entry", "java.util.regex.Pattern",
    ...
    public void testImportsForComplicatedTypes() {
      ...
      Set<TypeMirror> types = ImmutableSet.of(typeUtil.getPrimitiveType(TypeKind.INT),
          typeMirrorOf("java.util.regex.Pattern"), typeUtil.getDeclaredType(list,  // List<Timer>
      ...
      List<String> expectedImports =
          ImmutableList.of("java.math.BigInteger", "java.util.List", "java.util.Map",
              "java.util.Timer", "java.util.regex.Pattern");
      TypeSimplifier typeSimplifier = new TypeSimplifier(typeUtil, "foo.bar", types);
      assertEquals(expectedImports, ImmutableList.copyOf(typeSimplifier.typesToImport()));
    }
    ...
    public void testImportsForArrayTypes() {
      ...
      // Timer is referenced twice but should obviously only be imported once.
      List<String> expectedImports =
          ImmutableList.of("java.util.List", "java.util.Set", "java.util.Timer",
    ...
    public void testImportsForDefaultPackage() {
      Set<TypeMirror> types = ImmutableSet.of(typeUtil.getPrimitiveType(TypeKind.INT),
          typeMirrorOf(TEST_JAVA_CLASS_NAME), typeMirrorOf("java.lang.String"),
          typeMirrorOf("java.util.Map"), typeMirrorOf("java.util.Map.Entry"),
          typeMirrorOf("java.util.regex.Pattern"), typeMirrorOf("javax.management.MBeanServer"));
      List<String> expectedImports =
          ImmutableList.of("java.util.Map", "java.util.Map.Entry", "java.util.regex.Pattern",
              "javax.management.MBeanServer");
    ...
    public void testImportsForAmbiguousNames() {
      Set<TypeMirror> types =
          ImmutableSet.of(typeUtil.getPrimitiveType(TypeKind.INT), typeMirrorOf("java.awt.List"),
              typeMirrorOf("java.lang.String"), typeMirrorOf("java.util.List"),
              typeMirrorOf("java.util.Map"));
      List<String> expectedImports = ImmutableList.of("java.util.Map");
      TypeSimplifier typeSimplifier = new TypeSimplifier(typeUtil, "foo.bar", types);
      assertEquals(expectedImports, ImmutableList.copyOf(typeSimplifier.typesToImport()));
    }
    ...
    public void testSimplifyComplicatedTypes() {
      ...
      Set<TypeMirror> types = ImmutableSet.of(typeUtil.getPrimitiveType(TypeKind.INT),
          typeUtil.getArrayType(typeUtil.getPrimitiveType(TypeKind.BYTE)), pattern,
          typeUtil.getArrayType(pattern), typeUtil.getArrayType(typeUtil.getArrayType(pattern)),
          typeUtil.getDeclaredType(list, typeUtil.getWildcardType(null, null)),
          typeUtil.getDeclaredType(list, timer),
          typeUtil.getDeclaredType(map, typeUtil.getWildcardType(timer, null),
              typeUtil.getWildcardType(null, bigInteger))
      );
      List<String> expectedSimplifications =
          ImmutableList.of("int", "byte[]", "Pattern", "Pattern[]", "Pattern[][]", "List<?>",
              "List<Timer>", "Map<? extends Timer, ? super BigInteger>");
      TypeSimplifier typeSimplifier = new TypeSimplifier(typeUtil, "foo.bar", types);
